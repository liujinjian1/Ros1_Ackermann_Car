...
void my_car_control::car_pose(const nav_msgs::Odometry::ConstPtr &odomMsg)
{
    ...
    if (_pose > 9 && flag_go == 0) 
    {
        flag_go = 1;
       
    }
    if (_pose_red < 1.5 && flag_go == 1&&flag_red_first==0) 
    {
       
        
    }  
        if ((_pose_stop <= 8.7) && (flag_finish == 1)&& (flag_go==1)        ) //?D??¡¤¦Ì??¨ª¡ê3¦Ì??
    {
        cmd_vel.linear.x = 1500;
        cmd_vel.angular.z = 90;
        vel_pub.publish(cmd_vel);
        system("rosrun racecar launch_manager_node");   
    }
