...
int navigation_point=0;//导航点数量
int Stoplight1=0;//红绿灯第一次停车
int Stoplight2=0;//红绿灯第二次停车
int Stopcar=0;//停车场停车
int first_lap;//第一圈
int second_lap=0;//第二圈
int first_lap2;//第一圈
int second_lap2=0;//第二圈
int amcl_start=0;//开始导航
...
private:
  ros::NodeHandle n_;
  ros::Subscriber odom_sub, path_sub, goal_sub, encoder_sub, final_goal_sub, line_sub,traffic_light_sub;
...
L1Controller::L1Controller()
{
  ...
  pn.param("Stoplight1", Stoplight1, 0);
  pn.param("Stoplight2", Stoplight2, 0);
  pn.param("Stopcar", Stopcar, 0);
  pn.param("first_lap", first_lap, 0);
  pn.param("second_lap", second_lap, 0);
  pn.param("first_lap2", first_lap2, 0);
  pn.param("second_lap2", second_lap2, 0);
   pn.param("amcl_start", amcl_start, 0);
...
{ //目标点回调函数
  try
  {
    ...
    navigation_point = odom_goal.pose.position.z;
    goal_received = true;
    goal_reached = false;
...
void L1Controller::controlLoopCB(const ros::TimerEvent &)
{
  ...
  double encoder_pose = carPose.position.x;
  double dx2 = odom.twist.twist.linear.x;
  ...
  { 
    ...
    // double encoder_luchen = encoder_sudu*dt;
    // double luchen =0.0;
    // luchen+= sqrt(encoder_luchen*encoder_luchen);
    ...
    if (goal_received)
    {
      ...
      if (foundForwardPt)
      {
        if (!goal_reached)
        {
          ...
          if(navigation_point==Stoplight1 )
            {
              ...
              cmd_vel.linear.x = 1560;
              pub_.publish(cmd_vel);
              navigation_point=navigation_point+1;
            }
          
           if(navigation_point==Stoplight2 )
            {
         ...
         if(navigation_point==Stopcar )
          ...
          if(navigation_point==amcl_start )
            {
              ROS_INFO("Map saved successfully start navigation");
         ...
         //角度限幅
          ...
          if (cmd_vel.linear.x > vp_max_base+1500 && navigation_point>=first_lap && navigation_point<=second_lap)
            cmd_vel.linear.x = vp_max_base+1500;

          else if (cmd_vel.linear.x > vp_min+1500 )
            cmd_vel.linear.x = vp_min+1500;
